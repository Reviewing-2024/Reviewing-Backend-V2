 ...
 *
 * 흐름:
 * 1. Reader: SubCategory별로 페이지네이션하며 강의 크롤링 → InflearnCrawlingDto 반환
...
class InflearnCrawlingBatch(
    ...
    @Bean
    fun inflearnStep(inflearnReader: ItemStreamReader<InflearnCrawlingDto>): Step {
        return StepBuilder("inflearnCrawlingStep", jobRepository)
            .chunk<InflearnCrawlingDto, SubCategoryCourse>(20, platformTransactionManager)
    ...
    @Bean
    fun inflearnProcessor(): ItemProcessor<InflearnCrawlingDto, SubCategoryCourse> {
